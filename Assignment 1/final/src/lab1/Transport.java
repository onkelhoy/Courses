...
public abstract class Transport {
	...
	public Transport(String[] args){
		...
		if(args.length != 4){
			...
			try{
				...
				printErr("invalid buffer, port or message-rate"); //handle the errors but maybe not the best printout
			}
		}
	}
	...
	public void Verify(String newMSG){
		if (newMSG.compareTo(MSG) == 0)
			log.append(String.format("%dbytes sent and received msg are equal\n", newMSG.length()));
    	else
    		log.append(String.format("%dbytes sent and received msg not equal\n", newMSG.length()));
	}
